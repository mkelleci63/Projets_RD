...
def generer_tableau_adn(dossier_dna):
    """
    ...
    Colonnes : noms des échantillons (noms des dossiers).
    """
    data_adn = {
    ...
    echantillons = []
    for nom_echantillon in sorted(os.listdir(dossier_dna)):
        chemin_echantillon = os.path.join(dossier_dna, nom_echantillon)
...
def generer_tableau_arn(dossier_rna):
    """
    ...
    echantillons = []
    for nom_echantillon in sorted(os.listdir(dossier_rna)):
        chemin_echantillon = os.path.join(dossier_rna, nom_echantillon)
        if not os.path.isdir(chemin_echantillon):
            continue
        csv_file = None
        for f in os.listdir(chemin_echantillon):
...
def main(run_directory):
    """
    Génère un fichier Excel dans QC_Analyse/Taux_duplicats à partir des données des dossiers :
      - Logs_Intermediates/DnaDragenCaller/
      - Logs_Intermediates/RnaDragenCaller/
    """
    logs_intermediates = os.path.join(run_directory, 'Logs_Intermediates')
    dossier_dna = os.path.join(logs_intermediates, 'DnaDragenCaller')
    dossier_rna = os.path.join(logs_intermediates, 'RnaDragenCaller')
    ...
    with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
        ...
        # Format sans couleur pour la case (0,0)
        fmt_empty = workbook.add_format(dict(fmt_base))
        # Formats avec fonds pastel neutres
        fmt_header = workbook.add_format(dict(fmt_base, bold=True, bg_color='#BFD3C1'))
        # Ajout de text_wrap pour permettre un retour à la ligne
        ...
        # Hauteur de ligne fixée à 42,5 points (≈1,5 cm)
        row_height = 42.5
        # Largeur des colonnes fixée à environ 26,4 pour obtenir 5 cm (ou modifiez selon vos besoins)
        col_width = 26.4

        ######################
        # Feuille ADN
        if os.path.isdir(dossier_dna):
            df_adn = generer_tableau_adn(dossier_dna)
        ...
        ######################
        # Feuille ARN
        if os.path.isdir(dossier_rna):
            df_arn = generer_tableau_arn(dossier_rna)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage : python duplicats.py <chemin_vers_dossier_run>")
        sys.exit(1)
